...
def obtain_label(logger, val_loader, model, distance='cosine', threshold=0):
    ...
    all_logits = all_output.clone()
    all_output = nn.Softmax(dim=1)(all_output)
    _, predict = torch.max(all_output, 1)
    
    '''CE'''
    # criterion = torch.nn.CrossEntropyLoss(reduction='none')
    # confidence = -criterion(all_logits, predict.long()).numpy() 
    
    '''IE'''
    # entropy = -torch.sum(all_output * torch.log(all_output + 1e-8), dim=1)
    # confidence = -entropy.numpy() 
    
    '''softmax max probability'''
    confidence = all_output[torch.arange(all_output.size(0)), predict].cpu().numpy()
    ...
    return pred_label.astype('int'), confidence
...
def compute_knn_idx(logger, model, train_loader1, train_loader2, feat_memory1, feat_memory2, label_memory1, label_memory2, img_num1, img_num2, target_sample_num=2, topk=1, reliable_threshold=0.0):
    ...
    target_label, confidence = obtain_label(logger, train_loader2, model)
    target_label = torch.from_numpy(target_label).cuda()

    return target_label, knnidx, knnidx_topk, target_knnidx, confidence
...
def do_train_uda(cfg,
    ...
    # load CLIP
    model_clip, preprocess = open_clip.create_model_from_pretrained('hf-hub:laion/CLIP-ViT-L-14-laion2B-s32B-b82K')
    model_clip.to(device)
    model_clip.eval()
    # dataset with clip preprocess
    train_set2_clip = ImageDataset(t_dataset.train, preprocess)
    train_loader2_clip = DataLoader(
        train_set2_clip, batch_size=cfg.TEST.IMS_PER_BATCH,
        shuffle=False,
        num_workers=cfg.DATALOADER.NUM_WORKERS, collate_fn=train_collate_fn,
        pin_memory=True, persistent_workers=True,
    )
    # load resnet50 as feature extractor
    # feature_model = torch.nn.Sequential(*(list(models.resnet50(pretrained=True).children())[:-1]))
    ...
    for epoch in range(1, epochs + 1):
        ...
        if (epoch-1) % update_epoch == 0:
            ''' 1. update KNN pseudo labels '''
            logger.info(f"Updating feature memory at epoch {epoch}...")
            ...
            logger.info(f"Computing KNN pseudo labels with top-{dynamic_top} neighbors...")
            target_label, knnidx, knnidx_topk, target_knnidx, confidence_knn = compute_knn_idx(logger, model, train_loader1, train_loader2, feat_memory1, feat_memory2, label_memory1, label_memory2, img_num1, img_num2, topk=dynamic_top, reliable_threshold=0.0)
            
            """calculate the pseudo label accuracy of the small model"""
            logger.info("Computing small model pseudo label accuracy...")
            correct_final = 0
            total_final = 0
            with torch.no_grad():
                for batch_idx, (imgs, vid, _, _, idx) in enumerate(train_loader2):
                    img = imgs.to(device)  
                    ...
                    predicted_labels = target_label[idx]  
                    predicted_labels = torch.tensor(predicted_labels, dtype=torch.long, device=device)
                   
                    correct_final += (predicted_labels == vid).sum().item()
                    total_final += vid.size(0)
            
            accuracy_final = correct_final / total_final * 100  # 百分比
            print(f"small model pseudo label accuracy: {accuracy_final:.2f}%")
            logger.info(f"small model pseudo label accuracy: {accuracy_final:.2f}%")
            
            ''' 2. calculating the pseudo label accuracy of CLIP '''
            logger.info("Generating CLIP pseudo labels...")
            ...
            clip_features = []
            with torch.no_grad():
                for batch_idx, (imgs, vid, _, _, idx) in enumerate(train_loader2_clip):
                    img = imgs.to(device)  
                    labels = torch.tensor(vid)  
                    ...
                    '''softmax-max probability'''
                    initial_temperature =11.0  
                   
                    gamma = 0.001 
                    temperature = 1.0 + (initial_temperature - 1.0) * (gamma ** (epoch - 1))
                    # temperature = initial_temperature
                    # temperature = max(1.0, 1.0 + (initial_temperature - 1.0) * (1 - epoch / epochs))
                    
                    probs = (logits_per_image / temperature).softmax(dim=-1).cpu() 
                    
                    predicted_labels = probs.argmax(dim=-1)
                    
                    confidence = probs[torch.arange(probs.size(0)), predicted_labels]
                    
                    '''IE'''
                    # predicted_labels = probs.argmax(dim=-1)
                    # confidence = torch.sum(probs * torch.log(probs + 1e-8), dim=1) 
                    
                    '''CE'''
                    # predicted_labels = probs.argmax(dim=-1)
                    # criterion = torch.nn.CrossEntropyLoss(reduction='none')
                    # confidence = -criterion(logits_per_image.cpu(), predicted_labels.long())  
                    
                    correct += (predicted_labels == labels.cpu()).sum().item()
                    total += labels.shape[0]
                    
                    label_clip.append(predicted_labels)
                    confidence_clip.append(confidence)  
                    clip_features.append(image_features.cpu().numpy())
            
            label_clip = torch.cat(label_clip, dim=0).to(device)
            confidence_clip = torch.cat(confidence_clip, dim=0).cpu().numpy()
            clip_features = np.concatenate(clip_features, axis=0)
            
            accuracy = correct / total * 100  
            print(f"CLIP pseudo label accuracy: {accuracy:.2f}%")
            
            ''' 3. pseudo label selection '''
            # logger.info("Computing silhouette scores for classifier and CLIP pseudo labels...")
            # resnet_features = []
            # with torch.no_grad():
            #     for batch_idx, (imgs, _, _, _, idx) in enumerate(train_loader2):
            #         img = imgs.to(device) 
            #         feature = feature_model(img) 
            #         feature = feature.view(feature.size(0), -1)
            #         resnet_features.append(feature.cpu().numpy())
            # resnet_features = np.concatenate(resnet_features, axis=0)

            # computing silhouette scores
            # silhouette_values_resnet = silhouette_samples(resnet_features, label_memory2.cpu().numpy())
            # silhouette_values_clip = silhouette_samples(resnet_features, label_clip.cpu().numpy())
            # silhouette_values_resnet = silhouette_samples(clip_features, target_label.cpu().numpy())
            # silhouette_values_clip = silhouette_samples(clip_features, label_clip.cpu().numpy())
            # silhouette_values_resnet = silhouette_samples(feat_memory2.numpy(), target_label.cpu().numpy())
            # silhouette_values_clip = silhouette_samples(feat_memory2.numpy(), label_clip.cpu().numpy())

            # mask = silhouette_values_clip > silhouette_values_resnet
            mask = confidence_clip > confidence_knn
            
            # alpha = min(epoch / 20, 1.0) 
            alpha = 1.0
            random_mask = np.random.rand(mask.sum()) < alpha
            mask_indices = np.where(mask)[0] 
            selected_indices = mask_indices[random_mask]
            
            target_label[selected_indices] = label_clip[selected_indices]
            # target_label = label_clip
            logger.info(f"Updated {selected_indices.shape[0]} pseudo labels using CLIP!")
            print(f"replace {selected_indices.shape[0]} pseudo labels of small model with CLIP")
            
            logger.info("Computing final pseudo label accuracy...")
            correct_final = 0
            total_final = 0
            with torch.no_grad():
                for batch_idx, (imgs, vid, _, _, idx) in enumerate(train_loader2):
            ...
            accuracy_final = correct_final / total_final * 100
            print(f"final pseudo label accuracy: {accuracy_final:.2f}%")
            logger.info(f"Final pseudo label accuracy: {accuracy_final:.2f}%")
